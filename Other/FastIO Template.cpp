namespace IO { // Template taken from https://codeforces.com/contest/1299/submission/70935368
    ...
    inline void read_int(T &number, Args &... args) {
        read_int(number); read_int(args...);
    }
    inline void read_double(double &number) {
        bool negative = false; number = 0;
    ...
    inline void write_double(double number, char after = '\0', int places = 6) {
        if (number < 0) write_char('-'), number = -number;
        assert(number <= 9e18);
        ...
        if (places > 0) {
            ...
            if (floored != 0) {
                ...
                if (places % 2) number_buffer[length++] = floored + '0';
